        ...
        private async Task CashFree(CashFreeGetStatusObject statusObject, string request)
        {
            try
            {
                if (Methods.CheckConnectivity())
                {
                    ...
                    {
                        case 200:
                            ProgressDialogHelper.Dismiss(ActivityContext);
        ...
        private class MyWebViewClient : WebViewClient
        {
            ...
            public override async void OnPageStarted(WebView view, string url, Bitmap favicon)
            {
                try
                {
                    ...
                    {
                        //Show a progress
                        ProgressDialogHelper.Show(MActivity.ActivityContext, MActivity.ActivityContext.GetText(Resource.String.Lbl_Processing));
