...
namespace Facesofnaija.Payment
{
    public class InitPayPalPayment : Object, IDropInListener, IFetchMostRecentPaymentMethodCallback
    {
        ...
        public string Price, PayType, Id;
        ...
        //Paypal
        public void BtnPaypalOnClick(string price, string payType, string id = "")
        {
            try
            {
                Price = price; PayType = payType; Id = id;
        ...
        private DropInRequest InitPayPal()
        {
            try
            {
                ...
                paypalRequest.BillingAgreementDescription = name;
        ...
        public async void OnResult(DropInResult result, Java.Lang.Exception error)
        {
            try
            {
                ...
                if (result.PaymentMethodType != null || result.PaymentMethodType == DropInPaymentMethod.GooglePay || result.PaymentMethodType == DropInPaymentMethod.Paypal)
                {
                    ...
                    if (PayType == "membership")
                    {
                        if (ActivityContext is GoProActivity activity)
                        {
                            await activity.SetProAsync();
                        }
                    }
                    else if (PayType == "Funding")
                    {
                        if (ActivityContext is FundingViewActivity activity)
                        {
                            await activity.FundingPay();
                        }
                    }
                    else if (PayType == "AddFunds")
                    {
                        if (ActivityContext is TabbedWalletActivity activity)
                        {
                            activity.AddWallet();
                        }
                    }
