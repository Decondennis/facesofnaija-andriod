...
namespace Facesofnaija.Activities.Search.Fragment
{
    public class SearchCommunitiesFragment : AndroidX.Fragment.App.Fragment
    {
        #region Variables Basic

        public SearchCommunityAdapter MAdapter;
        ...
        private void SetRecyclerViewAdapters()
        {
            try
            {
                MAdapter = new SearchCommunityAdapter(Activity) { CommunityList = new ObservableCollection<CommunityDataObject>() };
        ...
        private void MainScrollEventOnLoadMoreEvent(object sender, EventArgs e)
        {
            try
            {
                ...
                if (item != null && !string.IsNullOrEmpty(item.CommunityId) && !MainScrollEvent.IsLoading)
                {
                    ContextSearch.OffsetGroup = item.CommunityId;
        ...
        private void MAdapterOnItemClick(object sender, SearchCommunityAdapterClickEventArgs e)
        {
            try
            {
                ...
                if (item != null)
                {
                    MainApplication.GetInstance()?.NavigateTo(Activity, typeof(CommunityProfileActivity), item);
                }
            }
            catch (Exception exception)
            {
        ...
        private void MAdapterOnJoinButtonItemClick(object sender, SearchCommunityAdapterClickEventArgs e)
        {
            try
            {
                var item = MAdapter.GetItem(e.Position);
                if (item != null)
                {
                    WoWonderTools.SetJoinGroup(Activity, item.CommunityId, e.Button);
                }
            }
            catch (Exception exception)
            {
