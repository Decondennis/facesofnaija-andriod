...
using Facesofnaija.Activities.NearbyShops;
...
namespace Facesofnaija.Activities.Market.Adapters
{
                                ...
                                if (viewHolder is ProductAdapterViewHolder holder)
                                {
                                    switch (item.Product?.Images?.Count)
                                    {
                                        case > 0 when item.Product.Images[0].Image.Contains("http"):
                                            ...
                                            break;
                                        case > 0:
                                            Glide.With(ActivityContext?.BaseContext).Load(new File(item.Product.Images?[0]?.Image)).Apply(new RequestOptions().CircleCrop().Placeholder(Resource.Drawable.ImagePlacholder_circle).Error(Resource.Drawable.ImagePlacholder_circle)).Into(holder.Thumbnail);
                                            break;
                                    }
                                ...
                                if (viewHolder is MyProductAdapterViewHolder holder)
                                {
                                    switch (item.Product?.Images?.Count)
                                    {
                                        case > 0 when item.Product != null && item.Product.Images[0].Image.Contains("http"):
                                            ...
                                            break;
                                        case > 0:
                                            Glide.With(ActivityContext?.BaseContext).Load(new File(item.Product?.Images?[0]?.Image)).Apply(new RequestOptions().CenterCrop().Placeholder(Resource.Drawable.ImagePlacholder).Error(Resource.Drawable.ImagePlacholder)).Into(holder.Thumbnail);
                                            break;
                                    }
