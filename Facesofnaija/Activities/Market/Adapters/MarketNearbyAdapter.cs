            ...
            try
            {
                switch (item.Images?.Count)
                {
                    case > 0 when item.Images[0].Image.Contains("http"):
                        ...
                        break;
                    case > 0:
                        Glide.With(ActivityContext?.BaseContext).Load(new File(item.Images?[0]?.Image)).Apply(new RequestOptions().CircleCrop().Placeholder(Resource.Drawable.ImagePlacholder_circle).Error(Resource.Drawable.ImagePlacholder_circle)).Into(holder.Thumbnail);
                        break;
                }
