...
namespace Facesofnaija.Activities.UsersPages.Adapters
{
    public class TouchImageAdapter : PagerAdapter
    {
        ...
        public override Object InstantiateItem(ViewGroup view, int position)
        {
            try
            {
                ...
                Glide.With(ActivityContext?.BaseContext).Load(ImagesList[position]).Apply(new RequestOptions()).Into(Image);
