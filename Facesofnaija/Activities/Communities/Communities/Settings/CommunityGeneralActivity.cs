...
namespace Facesofnaija.Activities.Communities.Communities.Settings
{
    ...
    public class CommunityGeneralActivity : BaseActivity//, MaterialDialog.IListCallback, MaterialDialog.ISingleButtonCallback
    {
        ...
        private string CommunitiesId, CategoryId = "", SubCategoryId = "", TypeDialog = "";
        private CommunityDataObject CommunityData;
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            try
            {
                ...
                var id = Intent?.GetStringExtra("CommunityId") ?? "Data not available";
                if (id != "Data not available" && !string.IsNullOrEmpty(id)) CommunitiesId = id;
        ...
        private async void TxtCreateOnClick(object sender, EventArgs e)
        {
            try
            {
                ...
                var (apiStatus, respond) = await RequestsAsync.Group.UpdateGroupDataAsync(CommunitiesId, dictionary);
                switch (apiStatus)
                {
                    case 200:
                        {
                                ...
                                case MessageObject result:
                                    {
                                        ...
                                        CommunityData.CommunityName = TxtUrl.Text;
                                        CommunityData.CommunityTitle = TxtTitle.Text;
                                        CommunityData.Username = TxtUrl.Text;
                                        CommunityData.About = TxtAbout.Text;
                                        CommunityData.CategoryId = CategoryId;
                                        CommunityData.Category = TxtCategories.Text;

                                        CommunityProfileActivity.CommunityDataClass = CommunityData;

                                        ToastUtils.ShowToast(this, GetText(Resource.String.Lbl_YourGroupWasUpdated), ToastLength.Short);

                                        Intent returnIntent = new Intent();
                                        returnIntent?.PutExtra("groupItem", JsonConvert.SerializeObject(CommunityData));
        ...
        //Get Data Community and set Categories
        private void Get_Data_Community()
        {
            try
            {
                CommunityData = JsonConvert.DeserializeObject<CommunityDataObject>(Intent?.GetStringExtra("CommunityData") ?? "");
                if (CommunityData != null)
                {
                    TxtTitle.Text = CommunityData.CommunityTitle;
                    TxtUrl.Text = CommunityData.Username;
                    TxtAbout.Text = Methods.FunString.DecodeString(CommunityData.About);
                    TxtCategories.Text = CategoriesController.ListCategoriesGroup.FirstOrDefault(categories => categories.CategoriesId == CommunityData.CategoryId)?.CategoriesName ?? CommunityData.Category;

                    CategoryId = CommunityData.CategoryId;

                    switch (string.IsNullOrEmpty(CommunityData.SubCategory))
                    {
                        case false:
                            {
                                var category = CategoriesController.ListCategoriesGroup.FirstOrDefault(categories => categories.CategoriesId == CategoryId)?.SubList.FirstOrDefault(sub => sub.CategoryId == CommunityData.SubCategory);
                                if (category != null)
                                {
                                    SubCategoriesLayout.Visibility = ViewStates.Visible;
