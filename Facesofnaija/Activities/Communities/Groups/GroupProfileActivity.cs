...
namespace Facesofnaija.Activities.Communities.Groups
{
    ...
    public class GroupProfileActivity : BaseActivity, IDialogListCallBack, AppBarLayout.IOnOffsetChangedListener, IActivityResultCallback, IDialogInputCallBack
    {
            ...
            try
            {
                if (p0 is CropImageView.CropResult result)
                {
                    if (result.IsSuccessful)
                    {
                        ...
                        {
                            string pathImg;
                            if (GalleryController?.ImageType == "Cover")
                            {
                                pathImg = filepath;
                                ...
                                UpdateImageGroup_Api(GalleryController?.ImageType, pathImg);
                            }
                            else if (GalleryController?.ImageType == "Avatar")
                            {
                                pathImg = filepath;
