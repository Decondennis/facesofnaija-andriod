...
namespace Facesofnaija.Activities.Tabbes
{
    ...
    public class TabbedMainActivity : BaseActivity, IDialogListCallBack, IActivityResultCallback
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            try
            {
                ...
                new Handler(Looper.MainLooper).PostDelayed(() => Task.Run(() => MainApplication.GetInstance()?.SecondRunExcite()), 1500);
                ...
                new Handler(Looper.MainLooper).PostDelayed(() => Task.Run(GetGeneralAppData), 2500);
                SetService();
            }
            catch (Exception e)
            {
        ...
        private void AddFragmentsTabs()
        {
            try
            {
                Log.Warn("FON_TIMELINE", "AddFragmentsTabs: START");
                ...
                if (TabAdapter != null && TabAdapter.ItemCount <= 0)
                {
                    ...
                    Log.Warn("FON_TIMELINE", $"AddFragmentsTabs: Adapter set, ItemCount={TabAdapter.ItemCount}");
                }
                else
                {
                    Log.Warn("FON_TIMELINE", $"AddFragmentsTabs: Skipped adapter setup, TabAdapter.ItemCount={TabAdapter?.ItemCount}");
                }
                ...
                Log.Warn("FON_TIMELINE", "AddFragmentsTabs: DONE");
            }
            catch (Exception exception)
            {
                Log.Warn("FON_TIMELINE", $"AddFragmentsTabs: EXCEPTION {exception.Message}");
                Methods.DisplayReportResultTrack(exception);
            }
        }
        ...
        private void GetGeneralAppData()
        {
            try
            {
                ...
                if (Methods.CheckConnectivity())
                    PollyController.RunRetryPolicyFunction(new List<Func<Task>> { () => ApiRequest.Get_MyProfileData_Api(this), ApiRequest.GetTimeZoneAsync, ApiRequest.LoadSuggestedUser, ApiRequest.LoadSuggestedGroup, ApiRequest.LoadSuggestedPage });
        ...
        {
            Android.Util.Log.Warn("FON_STORY_FLOW", $"FetchUserStoriesAsync userId={userId}");
            try
            {
