...
using static Facesofnaija.Activities.AddPost.FeelingActivitiesTemplate;
using static Facesofnaija.Activities.AddPost.PostDoingDialog;
...
namespace Facesofnaija.Activities.AddPost
{
    [Activity(Icon = "@mipmap/icon", Theme = "@style/MyTheme", Exported = true, ConfigurationChanges = ConfigChanges.Locale | ConfigChanges.UiMode | ConfigChanges.ScreenSize | ConfigChanges.Orientation | ConfigChanges.ScreenLayout | ConfigChanges.SmallestScreenSize)]
    ...
    public class PostSharingActivity : BaseActivity, IActivityResultCallback, SlidingPaneLayout.IPanelSlideListener, SlidingUpPanelLayout.IPanelSlideListener, IDialogListCallBack, IDialogInputCallBack, ITextWatcher, IOnFeelingClick, IOnDoingListener
    {
        ...
        private void TxtAddPostOnClick(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    //    //ProgressDialogHelper.ShowErrorToast(this, GetText(Resource.String.Lbl_Post_Failed), MaskType.Clear, TimeSpan.FromSeconds(1));
                    //}
            ...
            try
            {
                if (p0 is CropImageView.CropResult result)
                {
                    if (result.IsSuccessful)
                    {
                        ...
                        if (!string.IsNullOrEmpty(filepath) && ItemAttachmentsClick != null)
                        {
