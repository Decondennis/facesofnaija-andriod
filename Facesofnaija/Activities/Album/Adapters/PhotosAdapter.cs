...
namespace Facesofnaija.Activities.Album.Adapters
{
    public class PhotosAdapter : RecyclerView.Adapter, ListPreloader.IPreloadModelProvider
    {
        ...
        public override RecyclerView.ViewHolder OnCreateViewHolder(ViewGroup parent, int viewType)
        {
            try
            {
                ...
                var itemView = LayoutInflater.From(parent.Context)?.Inflate(Resource.Layout.Style_ImageAlbumView, parent, false);
    ...
    public class PhotosAdapterViewHolder : RecyclerView.ViewHolder
    {
        public PhotosAdapterViewHolder(View itemView, Action<PhotosAdapterClickEventArgs> clickListener, Action<PhotosAdapterClickEventArgs> longClickListener) : base(itemView)
        {
            try
            {
                ...
                //Event
                itemView.Click += (sender, e) => clickListener(new PhotosAdapterClickEventArgs { View = itemView, Position = BindingAdapterPosition });
                itemView.LongClick += (sender, e) => longClickListener(new PhotosAdapterClickEventArgs { View = itemView, Position = BindingAdapterPosition });
            }
            catch (Exception exception)
            {
                Methods.DisplayReportResultTrack(exception);
