...
using Facesofnaija.Activities.Suggested.Pages;
using Facesofnaija.Activities.Suggested.User;
using Facesofnaija.Activities.Tabbes;
using Facesofnaija.Activities.UserProfile.Adapters;
using Facesofnaija.Helpers.Ads;
using Facesofnaija.Helpers.Animinations;
...
using SuperTextView = Facesofnaija.Library.Anjo.SuperTextLibrary.SuperTextView;
...
namespace Facesofnaija.Activities.NativePost.Post
{
    public class AdapterHolders
    {
        ...
        public class PostVideoSectionViewHolder : RecyclerView.ViewHolder, View.IOnClickListener, IVideoPlayerEventListener
        {
            public View MainView { get; private set; }
            ...
            public StyledPlayerView ExoPlayer { get; set; }
            public StyledPlayerControlView StyledPlayerControlViews { get; set; }
            ...
            public PostVideoSectionViewHolder(View itemView, NativePostAdapter postAdapter) : base(itemView)
            {
                try
                {
                    ...
                    ExoPlayer = itemView.FindViewById<StyledPlayerView>(Resource.Id.itemVideoPlayer);
                    StyledPlayerControlViews = ExoPlayer.FindViewById<StyledPlayerControlView>(Resource.Id.exo_controller);
                    TextPositionControl = StyledPlayerControlViews.FindViewById<TextView>(Resource.Id.exo_position);
                    PostAdapter = postAdapter;
            ...
            public void OnPrePlay(IExoPlayer player)
            {
                try
                {
                    ExoPlayer.Visibility = ViewStates.Gone;
                    VideoImage.Visibility = ViewStates.Visible;
                    ...
                    //if (!GlobalFeedSettings.DisplayExoPlayerControllers)
                    //    ExoPlayer.UseController = false;


                    //StyledPlayerControlViews.Visibility = ViewStates.Gone;
                    // ExoPlayer.ControllerAutoShow =false;
                    //StyledPlayerControlViews.ShowTimeoutMs = 5000;
            ...
            public void OnPlayCanceled()
            {
                try
                {
                    if (ExoPlayer.Player != null)
                    {
                        ...
                        StyledPlayerControlViews.ProgressUpdate -= ControlView_ProgressUpdate;
                    }
                    ...
                    //Reset Layout Bug shows previos hight video
                    //ExoPlayer.VideoSurfaceView.Layout(0, 0, 0, 0);
                    //ExoPlayer.Layout(0, 0, 0, 0);
                    //MediaContainer.Layout(0, 0, 0, 0);
                    //StyledPlayerControlViews.Layout(0, 0, 0, 0);
                }
                catch (Exception e)
                {
                    Methods.DisplayReportResultTrack(e);
            ...
            public void OnPlay()
            {
                try
                {
                    if (!RepeatAniminationOnclicks)
                        return;

                    Animation fadeOut = new AlphaAnimation(1, 0);
                    fadeOut.Interpolator = new AccelerateInterpolator();
                    fadeOut.Duration = 400;
                    fadeOut.SetAnimationListener(new ImageFadeOutAnimationListener(VideoImage));
                    Action myAction = () =>
                    {
                        if (ExoPlayer.Player != null)
                        {
                            ExoPlayer.Visibility = ViewStates.Visible;
                            PlayButton.Visibility = ViewStates.Invisible;
                            VideoImage.StartAnimation(fadeOut);
                            RepeatAniminationOnclicks = false;
                        }
                    };

                    MainView.PostDelayed(myAction, 500L);
                }
                catch (Exception e)
                {
                    Methods.DisplayReportResultTrack(e);
            ...
            public void PlayVideo(IExoPlayer player)
            {
                try
                {
                    MediaItem mediaItem = MediaItem.FromUri(Android.Net.Uri.Parse(VideoUrl));

                    var cacheFactoryDataSource = WRecyclerView.GetInstance().ExoMediaController.CacheFactoryDataSource;
                    var mediaSource = new ProgressiveMediaSource.Factory(cacheFactoryDataSource).CreateMediaSource(mediaItem);
            ...
            public void ControlView_ProgressUpdate(object sender, StyledPlayerControlView.ProgressUpdateEventArgs e)
            {
                try
                {
                    var timeLeft = (e.BufferedPosition - e.Position);
            ...
            public Android.Net.Uri GetVideoUrl()
            {
                return Android.Net.Uri.Parse(VideoUrl);
            }

            public PostVideoSectionViewHolder GetViewHolderOfListener()
            {
                return this;
            }
        }
