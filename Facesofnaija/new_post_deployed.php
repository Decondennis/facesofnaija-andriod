...
register_shutdown_function(function () {
    ...
    if (!empty($fatal) && in_array($fatal['type'], array(E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR, E_USER_ERROR))) {
        $context = array(
            'type' => isset($_GET['type']) ? $_GET['type'] : null,
            'user_id' => isset($_POST['user_id']) ? $_POST['user_id'] : null,
            'has_postPhotos' => isset($_FILES['postPhotos']['name']) ? 1 : 0,
            'postPhotos_name_type' => isset($_FILES['postPhotos']['name']) ? gettype($_FILES['postPhotos']['name']) : 'none'
        );
        error_log('NEW_POST_FATAL: ' . json_encode($fatal) . ' CTX: ' . json_encode($context));
    }
});
    ...
    if (empty($json_error_data)) {
        ...
        if (empty($user_login_data)) {
            ...
            if (isset($_FILES['postPhotos']['name']) && empty($mediaFilename) && empty($_POST['album_name'])) {
                $post_photos_count = is_array($_FILES['postPhotos']['name']) ? count($_FILES['postPhotos']['name']) : 1;

                if ($post_photos_count == 1) {
                    $photo_name = is_array($_FILES['postPhotos']['name']) ? $_FILES['postPhotos']['name'][0] : $_FILES['postPhotos']['name'];
                    $photo_size = is_array($_FILES['postPhotos']['size']) ? $_FILES['postPhotos']['size'][0] : $_FILES['postPhotos']['size'];
                    $photo_tmp_name = is_array($_FILES['postPhotos']['tmp_name']) ? $_FILES['postPhotos']['tmp_name'][0] : $_FILES['postPhotos']['tmp_name'];
                    $photo_type = is_array($_FILES['postPhotos']['type']) ? $_FILES['postPhotos']['type'][0] : $_FILES['postPhotos']['type'];

                    if ($photo_size > $wo['config']['maxUpload']) {
                        $invalid_file = 1;
                    } else if (Wo_IsFileAllowed($photo_name) == false) {
                        $invalid_file = 2;
                    } else {
                        $fileInfo = array(
                            'file' => $photo_tmp_name,
            ...
            if (isset($_FILES['postPhotos']['name'])) {
                ...
                $post_photos_count = is_array($_FILES['postPhotos']['name']) ? count($_FILES['postPhotos']['name']) : 1;
                for ($i = 0; $i < $post_photos_count; $i++) {
                    if ($post_photos_count > 1) {
                        $photo_name = $_FILES['postPhotos']['name'][$i];
                    } else {
                        $photo_name = $_FILES['postPhotos']['name'];
                    }
                    $new_string = pathinfo($photo_name);
            ...
            if (empty($errors)) {
                ...
                } else if (!empty($wo['user']['community_id']) && is_numeric($wo['user']['community_id'])) {
                    $community_id = (int) $wo['user']['community_id'];
                }
                ...
                if ($id) {
                    ...
                    if (isset($_FILES['postPhotos']['name'])) {
                        if (is_array($_FILES['postPhotos']['name']) && count($_FILES['postPhotos']['name']) > 0) {
