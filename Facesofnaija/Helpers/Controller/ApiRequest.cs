                                ...
                                case GetUserDataObject result:
                                    {
                                        ...
                                        context?.RunOnUiThread(() =>
                                        {
                                            try
                                            {
                                                ...
                                                var tabbedActivity = context as TabbedMainActivity ?? TabbedMainActivity.GetInstance();
                                                var adapter = tabbedActivity?.NewsFeedTab?.PostFeedAdapter;
                                                if (adapter?.ListDiffer != null)
                                                {
                                                    var addPostItem = adapter.ListDiffer.FirstOrDefault(a => a.TypeView == PostModelType.AddPostBox);
                                                    if (addPostItem != null)
                                                        adapter.NotifyItemChanged(adapter.ListDiffer.IndexOf(addPostItem));
