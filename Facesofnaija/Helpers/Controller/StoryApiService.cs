        ...
        private static GetUserStoriesObject NormalizeStoriesResponse(string json)
        {
            ...
            try
            {
                ...
                if (storiesToken != null && int.TryParse(apiStatusToken?.ToString(), out var normalizedStatus) && normalizedStatus == 200)
                {
                    ...
                    if (normalizedStories.Count == 0 && storiesToken is JArray rawRows && rawRows.Count > 0)
                    {
                        ...
                        foreach (var rowToken in rawRows)
                        {
                            ...
                            var videos = row["videos"]?.ToObject<List<StoryDataObject.Video>>() ?? new List<StoryDataObject.Video>();
                            var images = row["images"]?.ToObject<List<StoryDataObject.Image>>() ?? new List<StoryDataObject.Image>();
