...
using System;
using Facesofnaija.Activities.ReelsVideo;
...
namespace Facesofnaija.Helpers.Utils
{
    public class BottomNavigationTab : Java.Lang.Object, View.IOnClickListener
    {
        ...
        private LinearLayout Tab, HomeLayout, NotificationLayout, TrendingLayout, ReelLayout, JobsLayout, VirtualTourLayout, MoreLayout;
        private ImageView ImageHome, ImageNotification, ImageTrending, ImageReel, ImageJobs, ImageVirtualTour, ImageMore;
        private View SelectHome, SelectNotification, SelectTrending, SelectReel, SelectJobs, SelectVirtualTour, SelectMore;
        ...
        private void Initialize()
        {
            try
            {
                ...
                JobsLayout = MainActivity.FindViewById<LinearLayout>(Resource.Id.llJobs);
                ImageJobs = MainActivity.FindViewById<ImageView>(Resource.Id.ivJobs);
                SelectJobs = MainActivity.FindViewById<View>(Resource.Id.selectJobs);

                VirtualTourLayout = MainActivity.FindViewById<LinearLayout>(Resource.Id.llVirtualTour);
                ImageVirtualTour = MainActivity.FindViewById<ImageView>(Resource.Id.ivVirtualTour);
                SelectVirtualTour = MainActivity.FindViewById<View>(Resource.Id.selectVirtualTour);
                ...
                VirtualTourLayout?.SetOnClickListener(this);
                ...
                {
                    JobsLayout.Visibility = ViewStates.Gone;
                    weightSum--;
                }

                if (!AppSettings.ShowVirtualTour)
                {
                    VirtualTourLayout.Visibility = ViewStates.Gone;
                    weightSum--;
                }
        ...
        public void SelectItem(int index)
        {
            try
            {
                ...
                ImageJobs.SetColorFilter(UnSelectColor);
                ImageVirtualTour.SetColorFilter(UnSelectColor);
                ...
                SelectJobs.Visibility = ViewStates.Gone;
                SelectVirtualTour.Visibility = ViewStates.Gone;
        ...
        public void OnClick(View v)
        {
            try
            {
                ...
                else if (v.Id == JobsLayout?.Id)
                {
                    MainActivity?.NewsFeedTab?.MainRecyclerView?.StopVideo();
                    OpenNewsFeedTab = 1;

                    Intent intent = new Intent(Intent.ActionView, Uri.Parse(AppSettings.JobsUrl));
                    intent?.AddFlags(ActivityFlags.NewTask);
                    MainActivity.StartActivity(intent);
                }
                else if (v.Id == VirtualTourLayout?.Id)
                {
                    MainActivity?.NewsFeedTab?.MainRecyclerView?.StopVideo();
                    OpenNewsFeedTab = 1;

                    Intent intent = new Intent(Intent.ActionView, Uri.Parse(AppSettings.VirtualTourUrl));
                    intent?.AddFlags(ActivityFlags.NewTask);
                    MainActivity.StartActivity(intent);
                }
