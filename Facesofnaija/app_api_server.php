...
if (!empty($_GET['application']) && $_GET['application'] == 'phone' && !empty($_GET['type']) && $_GET['type'] == 'new_post') {
    $diag_file = __DIR__ . '/new_post_router_diag.log';
    @header('X-NewPost-Router-Diag: 1');
    $diag_ctx = array(
        'time' => date('c'),
    ...
    @file_put_contents($diag_file, "ROUTER_ENTER " . json_encode($diag_ctx) . PHP_EOL, FILE_APPEND);

    register_shutdown_function(function () use ($diag_file) {
        $fatal = error_get_last();
        $row = array(
            'time' => date('c'),
            'last_error' => $fatal,
            'http_response_code' => function_exists('http_response_code') ? http_response_code() : null
        );
        @file_put_contents($diag_file, "ROUTER_SHUTDOWN " . json_encode($row) . PHP_EOL, FILE_APPEND);
    });
}
...
if ($application == 'windows_app') {
    ...
    switch ($type) {
        ...
        case 'block_user':
            include "api/$application/block_user.php";
            break;
        case 'get_my_community': //added this
        ...
        case 'request_community':
            include "api/$application/request_community.php"; //added this
            break;
        case 'create_event':
