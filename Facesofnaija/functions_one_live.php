...
function Wo_UploadImage($file, $name, $type, $type_file, $user_id = 0, $placement = '') {
    ...
    if ($type == 'cover') {
        if ($placement == 'page') {
            ...
            $query_one_delete_cover = mysqli_query($sqlConnect, " SELECT `cover` FROM " . T_COMMUNITIES . " WHERE `id` = " . $image_data['id'] . " AND `active` = '1'");
        } else if ($placement == 'event') {
        ...
        if (move_uploaded_file($file, $filename)) {
            ...
            if ($placement == 'page') {
                ...
                $update_data = Wo_UpdateCommunityData($image_data['id'], $image_data);
            } else if ($placement == 'event') {
        ...
        } elseif ($placement == 'community') {
            $user_data = Wo_CommunityData($image_data['id']);
        } else {
...
function Wo_RegisterNotification($data = array()) {
    ...
    $community_notifcation_query  = '';
    $community_notifcation_query2 = '';
    if (!empty($data['community_id']) && $data['community_id'] > 0) {
        $community = Wo_CommunityData($data['community_id']);
        if (!isset($community['id'])) {
        }
        $community_id                 = Wo_Secure($community['id']);
        $community_notifcation_query  = '`community_id`,';
        $community_notifcation_query2 = "{$community_id}, ";
    }
...
function Wo_GetSearch($search_qeury) {
    ...
    $query = mysqli_query($sqlConnect, " SELECT `id` FROM " . T_COMMUNITIES . " WHERE ((`community_name` LIKE '%$search_qeury%') OR `community_title` LIKE '%$search_qeury%') AND `active` = '1' LIMIT 3"); //added this
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
...
function Wo_ShareFile($data = array(), $type = 0, $crop = true) {
    ...
    $sharefile_diag = __DIR__ . '/../../sharefile_diag.log';
    $allowed = '';
    ...
    @file_put_contents($sharefile_diag, 'ENTER ' . json_encode(array(
        'time' => date('c'),
        'name' => isset($data['name']) ? $data['name'] : null,
        'size' => isset($data['size']) ? $data['size'] : null,
        'type' => isset($data['type']) ? $data['type'] : null,
        'upload_tmp_exists' => (!empty($data['file']) && file_exists($data['file'])) ? 1 : 0,
        'helper_type' => $type,
        'crop' => $crop ? 1 : 0
    )) . PHP_EOL, FILE_APPEND);
    $new_string     = pathinfo($data['name'], PATHINFO_FILENAME) . '.' . strtolower(pathinfo($data['name'], PATHINFO_EXTENSION));
    ...
    if ($data['is_video'] == 0) {
        ...
        $extension_allowed = explode(',', $allowed);
        if (!in_array($file_extension, $extension_allowed)) {
            @file_put_contents($sharefile_diag, 'REJECT_EXTENSION ' . $file_extension . PHP_EOL, FILE_APPEND);
            return false;
        }
    }
    if ($data['size'] > $wo['config']['maxUpload']) {
        @file_put_contents($sharefile_diag, 'REJECT_SIZE ' . $data['size'] . PHP_EOL, FILE_APPEND);
        return false;
    }
    ...
    if ($data['is_video'] == 0) {
        ...
        if (!in_array($data['type'], $mime_types)) {
            @file_put_contents($sharefile_diag, 'REJECT_MIME ' . $data['type'] . PHP_EOL, FILE_APPEND);
            return false;
        }
    }
    ...
    @file_put_contents($sharefile_diag, 'BEFORE_MOVE ' . json_encode(array('target' => $filename, 'folder' => $folder, 'fileType' => $fileType)) . PHP_EOL, FILE_APPEND);
    if (move_uploaded_file($data['file'], $filename)) {
        @file_put_contents($sharefile_diag, 'AFTER_MOVE_OK ' . $filename . PHP_EOL, FILE_APPEND);
        if ($second_file == 'jpg' || $second_file == 'jpeg' || $second_file == 'png' || $second_file == 'gif') {
            $check_file = getimagesize($filename);
            @file_put_contents($sharefile_diag, 'AFTER_GETIMAGESIZE ' . json_encode(array('ok' => !empty($check_file), 'file' => $filename)) . PHP_EOL, FILE_APPEND);
            if (!$check_file) {
                unlink($filename);
            }
            if ($crop == true) {
                if ($type == 1) {
                    if ($second_file != 'gif') {
                        @file_put_contents($sharefile_diag, 'BEFORE_COMPRESS_SMALL ' . $filename . PHP_EOL, FILE_APPEND);
                        @Wo_CompressImage($filename, $filename, 50);
                        @file_put_contents($sharefile_diag, 'AFTER_COMPRESS_SMALL ' . $filename . PHP_EOL, FILE_APPEND);
                    }
                    $explode2  = @end(explode('.', $filename));
                    $explode3  = @explode('.', $filename);
                    $last_file = $explode3[0] . '_small.' . $explode2;
                    @file_put_contents($sharefile_diag, 'BEFORE_RESIZE_CROP ' . json_encode(array('src' => $filename, 'dest' => $last_file)) . PHP_EOL, FILE_APPEND);
                    if (Wo_Resize_Crop_Image(400, 400, $filename, $last_file, 60)) {
                        @file_put_contents($sharefile_diag, 'AFTER_RESIZE_CROP_OK ' . $last_file . PHP_EOL, FILE_APPEND);
                        if ($second_file != 'gif' && $wo['config']['watermark'] == 1 && !empty($wo['add_watermark']) && $wo['add_watermark'] == true) {
                            @file_put_contents($sharefile_diag, 'BEFORE_WATERMARK_SMALL ' . $last_file . PHP_EOL, FILE_APPEND);
                            watermark_image($last_file);
                            @file_put_contents($sharefile_diag, 'AFTER_WATERMARK_SMALL ' . $last_file . PHP_EOL, FILE_APPEND);
                        }
                    ...
                    if (!isset($data['compress']) && $second_file != 'gif') {
                        @file_put_contents($sharefile_diag, 'BEFORE_COMPRESS_MAIN ' . $filename . PHP_EOL, FILE_APPEND);
                        @Wo_CompressImage($filename, $filename, 10);
                        @file_put_contents($sharefile_diag, 'AFTER_COMPRESS_MAIN ' . $filename . PHP_EOL, FILE_APPEND);
                    }
                }
            }
            if ($second_file != 'gif' && $wo['config']['watermark'] == 1 && !empty($wo['add_watermark']) && $wo['add_watermark'] == true) {
                @file_put_contents($sharefile_diag, 'BEFORE_WATERMARK_MAIN ' . $filename . PHP_EOL, FILE_APPEND);
                watermark_image($filename);
                @file_put_contents($sharefile_diag, 'AFTER_WATERMARK_MAIN ' . $filename . PHP_EOL, FILE_APPEND);
            }
        }
        if (!empty($data['crop'])) {
            @file_put_contents($sharefile_diag, 'BEFORE_CUSTOM_CROP ' . $filename . PHP_EOL, FILE_APPEND);
            $crop_image = Wo_Resize_Crop_Image($data['crop']['width'], $data['crop']['height'], $filename, $filename, 60);
            @file_put_contents($sharefile_diag, 'AFTER_CUSTOM_CROP ' . json_encode(array('result' => !empty($crop_image), 'file' => $filename)) . PHP_EOL, FILE_APPEND);
        }
        if (empty($data['local_upload'])) {
        ...
        @file_put_contents($sharefile_diag, 'RETURN_OK ' . json_encode($last_data) . PHP_EOL, FILE_APPEND);
        return $last_data;
    }
    @file_put_contents($sharefile_diag, 'MOVE_FAILED ' . json_encode(array('tmp' => $data['file'], 'target' => $filename)) . PHP_EOL, FILE_APPEND);
}
...
function Wo_GetPosts($data = array('filter_by' => 'all', 'after_post_id' => 0, 'page_id' => 0, 'group_id' => 0, 'community_id' => 0, 'publisher_id' => 0, 'limit' => 5, 'event_id' => 0, 'ad-id' => 0)) {
    global $wo, $sqlConnect;
    ...
    if (!empty($data['community_id']) && is_numeric($data['community_id']) && $data['community_id'] > 0) { //added this
        $data['community_id']   = Wo_Secure($data['community_id']);
        $Wo_community_publisher = Wo_CommunityData($data['community_id']);
    }
    ...
    if (isset($Wo_publisher['user_id'])) {
    ...
    } else if (isset($Wo_community_publisher['id'])) { // added this
        $community_id = Wo_Secure($Wo_community_publisher['id']);
        $query_text .= " AND (`community_id` = {$community_id}) AND `id` NOT IN (SELECT `post_id` from " . T_PINNED_POSTS . " WHERE `community_id` = {$community_id})";
        switch ($data['filter_by']) {
            case 'text':
                $query_text .= " AND `postText` <> '' AND `postFile` = '' AND `postYoutube` = '' AND `postFacebook` = ''  AND `postVimeo` = ''  AND `postDailymotion` = '' AND `postSoundCloud` = '' ";
                break;
        ...
        if (!$wo['loggedin'] || $Wo_community_publisher['user_id'] != $wo['user']['id']) {
            $query_text .= " AND `postPrivacy` <> '3'";
        }
    ...
    if ((!isset($data['publisher_id']) || $data['publisher_id'] == $user) && empty($Wo_page_publisher['page_id']) && empty($Wo_group_publisher['id']) && empty($Wo_community_publisher['id'])) {
...
function Wo_PinPost($post_id = 0, $type = '', $id = 0) {
    ...
    if ($type == 'page') {
        ...
        /*if (Wo_IsCommunityModerator($id) === false) {
            return false;
        }*///coming back
        $where_delete_query = " WHERE `community_id` = {$id} AND `active` = '1'";
        $where_insert_query = " (`community_id`, `post_id`, `active`) VALUES ({$id}, {$post_id}, '1')";
    } else if ($type == 'event') {
